...
const MyTransaction = () => {
  ...
  const [allTransactions, setAllTransactions] = useState([]); // Store original data
      ...
      if (user?.email) {
        try {
          ...
          const res = await fetch(
            `${BASE_URL}/transactions?email=${user.email}`,
            {
  ...
  const applySort = (data, sortValue) => {
    let sorted = [...data];
  ...
  const handleSortChange = (value) => {
    setSortType(value);
    applySort(allTransactions, value);
  };
      ...
      if (result.isConfirmed) {
        try {
          ...
          if (data.deletedCount) {
            ...
            // Update both states
            const updatedAllTransactions = allTransactions.filter((t) => t._id !== _id);
            setAllTransactions(updatedAllTransactions);
            applySort(updatedAllTransactions, sortType);
          } else {
  ...
  const handleUpdateSubmit = async (e) => {
    ...
    try {
      const token = await user.getIdToken();
      const res = await fetch(
        `${BASE_URL}/transactions/${selectedTransaction._id}`,
        {
          method: "PATCH",
          headers: { 
    ...
    >
      ...
      >
        ...
        <p className="mt-3 text-lg text-green-100/90 font-medium">
          Track your income & expenses effortlessly — FinEase.
        </p>
      </div>
